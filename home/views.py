...
def index(request):
    if request.session.has_key('is_logged'):
        ...
        today_expense = addmoney_info.filter(Date=todays_date, add_money="Expense").aggregate(Sum('quantity'))['quantity__sum'] or 0
        
        # Calculate weekly expense
        one_week_ago = todays_date - datetime.timedelta(days=7)
        weekly_expense = addmoney_info.filter(Date__gte=one_week_ago, Date__lte=todays_date, add_money="Expense").aggregate(Sum('quantity'))['quantity__sum'] or 0
        
        # Calculate monthly expense
        one_month_ago = todays_date - datetime.timedelta(days=30)
        monthly_expense = addmoney_info.filter(Date__gte=one_month_ago, Date__lte=todays_date, add_money="Expense").aggregate(Sum('quantity'))['quantity__sum'] or 0
        
        # Calculate yearly expense
        one_year_ago = todays_date - datetime.timedelta(days=365)
        yearly_expense = addmoney_info.filter(Date__gte=one_year_ago, Date__lte=todays_date, add_money="Expense").aggregate(Sum('quantity'))['quantity__sum'] or 0
        
        # Calculate total income
        total_income = addmoney_info.filter(add_money="Income").aggregate(Sum('quantity'))['quantity__sum'] or 0
        

        sum = addmoney_info.filter(add_money="Amount").aggregate(Sum('quantity'))['quantity__sum'] or 0
